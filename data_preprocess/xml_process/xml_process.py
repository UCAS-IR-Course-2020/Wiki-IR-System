...
class Process_xml():
    def __init__(self,xml_file_list,save_dir,top_n=-1,one_file_save_max=2000,file_count_start=0):
        ...
        self.file_count=file_count_start
        self.word_id_dict={}
    ...
    ## Get every article info from xml
    def get_page(self):
    ...
    def creat_title_id_map_dict(self):
        ...
        output={}
        for i in range(26):
            output[chr(i + ord('A'))]=open(os.path.join(self.save_dir, 'word_id_map',chr(i + ord('A')))+'.txt', 'w', encoding='utf-8')
        other_char=open(os.path.join(self.save_dir, 'word_id_map','other'+'.txt'), 'w', encoding='utf-8')
            ...
            with open(os.path.join(self.save_dir,file),'r', encoding='utf-8') as f:
                for data in f.readlines():
                    ...
                    if(word[0].encode('utf-8').isalpha()):
                        output[word[0].upper()].write(word+'\t'+id+'\n')
                    else:
                        other_char.write(word+'\t'+id+'\n')

        for i in range(26):
            output[chr(i + ord('A'))].close()
        other_char.close()
            ...
            with open(os.path.join(self.save_dir,file),'r', encoding='utf-8') as f:
                for data in f.readlines():
                    ...
                    for link_word in page_link:
                        if(link_word!=''):
                            page_link_id=self.title2id(link_word)
