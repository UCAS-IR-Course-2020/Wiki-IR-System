...
import jieba
'''
目前查询过程耗时较长 (约1min), 主要因为VSM中向量维度太高 (词汇表太大, 文档数量多), 
考虑如下几种优化策略:
    1. 考虑制订df阈值, 忽略掉太过罕见的词项, 以防止向量过于稀疏, 非常罕见的词项可结合布尔查询进行检索
    2. 首先进行布尔查询, 对布尔查询的部分结果进行VSM查询
    3. 结合pagerank, 提前结束查询

文档间相似度计算用点积代替余弦距离, 余弦距离对长文本过于不友好, 因为query一般都很短, 
使用余弦距离会使得一些只出现query中一两个词的短文本, 反而排在前面
...
if __name__ == '__main__':
    ...
    # hint_res = en_query.complete_query('g')
    #
